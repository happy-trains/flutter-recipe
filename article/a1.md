...
Our repository implementation delegates the fulfillment of requests to data sources. 
...
[`RecipesRemoteDataSourceImpl`](../lib/features/search_recipe/data/datasources/recipes_remote_data_source.dart) uses `Client` provided by the `typesense` package to retrieve the data. `typesense` either returns a response or throws an exception of type `TypesenseException`. The methods in `RecipesRemoteDataSourceImpl` throw `ServerException` for all `TypesenseException`s.
...
[`RecipesLocalDataSourceImpl`](../lib/features/search_recipe/data/datasources/recipes_local_data_source.dart) uses [`shared_preferences`](https://pub.dev/packages/shared_preferences) to persist the data.
