...
Learn more about what’s happening with `implements UseCase<A, B>` [here](https://resocoder.com/2019/09/02/flutter-tdd-clean-architecture-course-3-domain-layer-refactoring/#UseCase_Base_Class). Implementing `call` function allows us to allow an instance of our class to be called like a function. 
...
> * A class should be a noun (`Person`) and it should have methods which that should be verbs (`chillOut`). 
...
```
 ...
 emits a state with `SearchStatus.failure` when `prunedQuery` function reutrns `Left(InvalidInputFailure())`
