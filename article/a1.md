...
/* write about the structure being follwed*/
...
`RecipesRemoteDataSourceImpl` uses `Client` provided by the `typesense` package to retrieve the data. `typesense` either returns a response or throws an exception of type `TypesenseException`. The m in `RecipesRemoteDataSourceImpl` throw `ServerException` for all `TypesenseException`s.
